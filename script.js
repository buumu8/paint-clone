...
function clearCanvas() {
  ...
  drawnArray = [];
  // File Name
  fileNameEl.textContent = "Paint Tools";
  // Active Tool
...
loadStorageBtn.addEventListener("click", () => {
  if (localStorage.getItem("savedCanvas")) {
    ...
    activeToolEl.textContent = "No Canvas Founded";
    brushTimeSetTimeout(BRUSH_TIME);
  }
});
...
downloadBtn.addEventListener("click", () => {
  ...
  const filename = fileNameEl.textContent
    ? `${fileNameEl.textContent.toLowerCase().replaceAll(" ", "-")}.jpeg`
    : "example-paint.jpeg";

  downloadBtn.download = filename;
  // Active Tool
  activeToolEl.textContent = `Image File Saved: ${filename}`;
  brushTimeSetTimeout(BRUSH_TIME);
});
